 ...
 * unless a time out is specified. Thesamples.clear() subscriber will receive data and output the
        ...
        try
        {
            ...
            while(!e.terminated.getTriggerValue() && !timedOut)
            {
                ...
                else if(burstInterval > 0)
                {
                    ...
                    burstStart = time + ((burstInterval - deltaTime) * ExampleUtilities.US_IN_ONE_MS);
                    burstCount = 0;
                }
                else
                {
