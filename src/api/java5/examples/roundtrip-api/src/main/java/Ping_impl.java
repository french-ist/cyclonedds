...
//                    Set<InstanceHandle> set = e.writer.getMatchedSubscriptions();
//                    while (set.isEmpty()) {
//                        set = e.writer.getMatchedSubscriptions();
//                    }
                    PublicationMatchedStatus status = e.writer.getPublicationMatchedStatus();
                    ...
                    //InstanceHandle handle = e.writer.lookupInstance(data);
                    //e.writer.dispose(handle);    
                    // Workaround waiting fix of JMOB-38 : java.lang.RuntimeException: Unable to encode data
            ...
            Duration waitTimeout = e.env.getSPI().newDuration(1, TimeUnit.SECONDS);
            data = new RoundTripModule.DataType(new byte[payloadSize]);
            for (int i = 0; i < payloadSize; i++) {
                data.payload[i] = 'a';
            }

            List<Sample<RoundTripModule.DataType>> samples = new ArrayList<Sample<RoundTripModule.DataType>>(100000);

            startTime = ExampleUtilities.GetTime();
            System.out.println("# Warming up...");
            boolean warmUp = true;
            while (!e.terminated.getTriggerValue() && (ExampleUtilities.GetTime()
                ...
                try {
                    ...
                    //System.out.println(">>>> TIMED-OUT (" + waitTimeout + "s) on take !!!");
                }
            }
            ...
            for (long i = 0; !e.terminated.getTriggerValue() && (numSamples == 0 || i < numSamples); i++) {
                /**
                ...
                try {
                    e.waitSet.waitForConditions(waitTimeout);
                    DataState states = e.subscriber.createDataState()
                            .withAnySampleState().withAnyViewState()
                            .withAnyInstanceState();
                    Selector<RoundTripModule.DataType> selector = e.reader.select().dataState(states);
                    /**
                    ...
                    //selector.maxSamples(ResourceLimits.LENGTH_UNLIMITED);
                    e.reader.take(samples);
                    //Sample.Iterator<RoundTripModule.Sample> it = e.reader.take(selector);
                    postTakeTime = ExampleUtilities.GetTime();
                    if (!e.terminated.getTriggerValue()) {
                        if (samples.size() != 1) {
                            //if (samples.size() != 1) {
                            Exception exception = new Exception(
                                    // "ERROR: Ping received " + it_size + " samples but "
                                    "ERROR: Ping received " + samples.size() + " samples but "
                    ...
                    if ((postTakeTime - startTime) > ExampleUtilities.US_IN_ONE_SEC || (i >= 1 && i == numSamples)) {
                        System.out.printf("%9d %9d %8.0f %8d %10d %8.0f %8d %10d %8.0f %8d%n",
                                elapsed + 1,
                                roundTrip.count,
                                roundTrip.GetMedian(),
                                roundTrip.min,
            ...
            if (!warmUp) {
                 ...
                 * Print overall stats
                 */
                System.out.printf("%n%9s %9d %8.0f %8d %10d %8.0f %8d %10d %8.0f %8d%n",
                        "# Overall",
