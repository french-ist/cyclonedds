...
class Subscriber_impl
{
    ...
    public void run (String args[])
    {
        try
        {
            ...
            /** Initialize entities */
            final SubEntities e = new SubEntities(partitionName);
            ...
            List<Sample<ThroughputModule.DataType>> samples = new ArrayList<Sample<ThroughputModule.DataType>>(100000);
            ...
            long cycles = 0;
            while(!e.terminated.getTriggerValue() && (maxCycles == 0 || cycles < maxCycles))
            {
                ...
                for(int i = 0; !e.terminated.getTriggerValue() && i < samples.size(); i++)
                {
                    if (samples.get(i).getData() != null)
                    {
                        long ph = samples.get(i).getInstanceHandle().hashCode(); 
                ...
                if(time > prevTime + ExampleUtilities.US_IN_ONE_SEC)
                {
                    ...
                    prevCount = new HashMap<Long, Long>(count);
